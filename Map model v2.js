...
function fundSA(sheetName){
  ...
  for (let k=2; k<dataRangeValues.length; k++){
    if (x >= 1){
      if (dataRangeValues[k][6]>=minScr) { //checks if projects meets minimum score requirement.
        ...
        inputSheet.getRange(k+1,inputSheet.getLastColumn()).setValue('Skip MinScr').setHorizontalAlignment('normal');
      }
    } else {
...
function ruralSort(){
  ...
  loopRHS: for (const row of rhsAmValues) {
    if (
      ...
      sACounter.get("Rural") >= 1
    ) {
      //Checking if there is more than $1 in RHS & HOME credit counter and in Rural Counter.
      if (row[6] >= minScr) {
        if (chkRrlHTisNeg(row) === false) {
          //false means there is more than $1 and Hsg type is not negative or it is not a set-aside type
          chkRrlHTAndSetCounters(row);
          inputSheet
            .createTextFinder(row[0])
            .findNext()
            .offset(0, 26)
            .setValue("Fund")
            .setHorizontalAlignment("normal")
            .getDataRegion(SpreadsheetApp.Dimension.COLUMNS)
            .setBackground("#9bbb59");
          ...
          //Large Family High Resource HT is negative
          resetLFH(row); //code to change "High/Highest Opportunity Area" column to "Y/N" and deduct 5%-10% from TB and rerun rural sort RHS/Native sort
          break loopRHS; //break is applied to stop the initial loop from continuing. The ruralSort function initiated by resetLFH function takes the sort through.
        } else if (chkRrlHTisNeg(row) === true) {
          //true means there is less than $1 and Hsg type is negative.
          inputSheet
            .createTextFinder(row[0])
            .findNext()
            .offset(0, 26)
  ...
  let nativeAmValues = dataRangeValues.filter(
    (item) => nativeAmRegEx.test(item[8]) && !item[lastCol - 1]
  ); //Filtering for Native American  projects and is not funded or skipped
  loopNtvAm: for (const row of nativeAmValues) {
    if (sACounter.get("Native American") >= 1 && sACounter.get("Rural") >= 1) {
      //Checking if there is more than $1 in Native American credit counter and in Rural Counter.
      if (row[6] >= minNAScr) {
        if (chkRrlHTisNeg(row) === false) {
          //false means there is more than $1 and Hsg type is not negative or it is not a set-aside type.
          chkRrlHTAndSetCounters(row);
          ...
          //Large Family High Resource HT is negative
          resetLFH(row); //code to change "High/Highest Opportunity Area" column to "Y/N" and deduct 5%-10% from TB and rerun rural sort RHS/Native sort
          break loopNtvAm; //break is applied to stop the initial loop from continuing. The ruralSort function initiated by resetLFH function takes the sort through.
        } else if (chkRrlHTisNeg(row) === true) {
          //true means there is less than $1 and Hsg type is negative.
          inputSheet
            .createTextFinder(row[0])
...
function ruralOtherSort(){
  ...
  loopOthrVals: for (const row of otherValues) {
        ...
        if (chkRrlHTisNeg(row) === false) { //false means there is more than $1 and Hsg type is not negative or it is not a set-aside type
          ...
          if(chkRestofData(row, inputSheet) === true) { //checks rest of data set and inputs "skip HT" if it finds project with a different Hsg type and same or higher score.
            if(row[2]==='Large Family' && row[14]==='Yes') { // Checks Large Family HT in High/Highest Resource Area
              resetLFH(row) //code to change "High/Highest Opportunity Area" column to "Y/N" and deduct 5%-10% from TB and rerun ruralOther sort
              break loopOthrVals; //break is applied to stop the initial loop from continuing. The ruralSort function initiated by resetLFH function takes the sort through.
            } else {
...
function chkRrlHTisNeg(dataRangeValue){
  if(dataRangeValue[2] == 'Large Family' && dataRangeValue[14] == 'Yes'){
    if(ruralCounter.get('Large Family High Opportunity')<= 1){
      //if Native American or RHS return 'reset LFHg' else return true for other rural sort
      let nativeAmRegEx = new RegExp('Native American','i');
      let sectionRegEx = new RegExp('Section','i');
      let homeRegex = new RegExp('HOME');
      if(nativeAmRegEx.test(dataRangeValue[8] || sectionRegEx.test(dataRangeValue[8] || homeRegex.test[8]))) {
        return 'reset LFHg'
      }
      return true;
    } else{
...
//Define reset Large Family High/Highest resource reset function: change "High/Highest Opportunity Area" column to "Y/N" and deduct 5%-10% from TB and rerun ruralOther sort
function resetLFH(dataRow){
  let inputsheet = ss.getSheetByName('Rural');
  let rowNum = inputsheet.createTextFinder(dataRow[0]).findNext().getRow();
  let lastCol = dataRow.length;
  let newTBScore = dataRow[7]-dataRow[lastCol-3];
  let nativeAmRegEx = new RegExp("Native American", "i");
  let sectionRegEx = new RegExp("Section", "i");
  let homeRegex = new RegExp("HOME");
  inputsheet.getRange(rowNum,15).setValue('Yes/No');
  inputsheet.getRange(rowNum, 8).setValue(`${newTBScore}`);
  // inputsheet.getDataRange().offset(2,0).sort([{column: 7, ascending: false},{column: 8, ascending: false}]);
...
function geoRegionSort(){
  ...
  while (runCycle <= numCycles) {
    for (const element of fundDirArr) {
      ...
      function fundGeo (currP) { // currP is the current project/position referencing the i in the for loop
        // ** break statements cannot jump function boundary **
        if (!dataRangeValues[currP][lastCol-1]) { //Project is not Funded or Skipped
          if (dataRangeValues[currP][6] >= minScr) { //Project meets or exceeds min score.
            if(dataRangeValues[currP][5]<=balanceAmt){
              if(regionCounter.get(element) > 0) { /**checks if region has run out of Credits.*/
                if (dataRangeValues[currP][6]>=dataRangeValues[currP-1][6]) { // checks if current project's score is equal to or greater than the previous projects score.
                ...
                } else { // if current projects score is not equal to or greater than previous project. Will fund Skipped HT projects before funding current project/position
                  let skipHTwCurrArr = dataRangeValues.filter(row => row[lastCol - 1] === 'Skip H/T');
                  skipHTwCurrArr.push(dataRangeValues[currP])
                  skipHTwCurrArr.forEach(item => {
                    if (item[5] <= balanceAmt) {
                      if (regionCounter.get(element) > 0) {
                        chkHTAndSetCounters(item);
                        inputsheet.createTextFinder(item[0]).findNext().offset(0, lastCol - 1).setValue('Fund skpdH/T>Scr').setHorizontalAlignment('normal').getDataRegion(SpreadsheetApp.Dimension.COLUMNS).setBackground('#9bbb59');
                        ss.getSheetByName('Funded Projects').appendRow(item);
                        balanceAmt -= item[5];
                        numDealFunded++
                      } else {
            ...
            inputsheet.getRange(currP + 1, inputsheet.getLastColumn()).setValue("Skip MinScr").setHorizontalAlignment("normal");
            dataRange = inputsheet.getDataRange();
            dataRangeValues = dataRange.getValues();
          }
        }
